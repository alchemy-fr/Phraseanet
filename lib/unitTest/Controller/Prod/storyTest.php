...
class storyTest extends PhraseanetWebTestCaseAuthenticatedAbstract
{
  ...
  public function testAttachStoryToWZ()
  {
    ...
    try
    {
      $this->client->request('POST', $goodRoute);
    ...
    //attach JSON
    $this->client->request('POST', $goodRoute, array(), array(), array(
        "HTTP_ACCEPT" => "application/json")
    );
  ...
  public function testDetachStoryFromWZ()
  {
    ...
    //attach
    $attachRoute = sprintf("/story/%s/%s/attach/", $story->get_sbas_id(), $story->get_record_id());
    $this->client->request('POST', $attachRoute);
