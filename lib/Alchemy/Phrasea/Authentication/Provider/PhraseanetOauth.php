...
class PhraseanetOauth extends AbstractProvider
{
    ...
    {
        if (!$this->session->has($this->id . '.provider.state')) {
            throw new NotAuthenticatedException('No state value in session ; CSRF try ?');
        }

        if ($request->query->get('state') !== $this->session->remove($this->id . '.provider.state')) {
        ...
        $this->session->remove($this->id . '.provider.state');
        $this->session->set($this->id . '.provider.access_token', $data['access_token']);

        try {
            $request = $this->client->get($this->id . '/user');
        ...
        $this->session->set($this->id . '.provider.id', $data['id']);
    }
    ...
    {
        if ('' === trim($this->session->get($this->id . '.provider.id'))) {
            throw new NotAuthenticatedException($this->id . ' has not authenticated');
        }

        return new Token($this, $this->session->get($this->id . '.provider.id'));
    }
    ...
    public function getIdentity()
    {
        ...
        try {
            $request = $this->client->get($this->id . '/user');
            $request->getQuery()->add('access_token', $this->session->get($this->id . '.provider.access_token'));
            $request->setHeader('Accept', 'application/json');
