...
//						continue;
      ...
      if (in_array('caption-yaml', $subdefs))
      {
        ...
        $desc = self::get_caption($download_element->base_id, $download_element->record_id, $session->ses_id, true, 'yaml');
  ...
  public static function get_caption($bas, $rec, $ses_id, $check_rights=true, $format = 'xml')
  {
          ...
          if (($rights || !isset($restrict[$ki])))
          {
            switch ($format)
            {
              ...
              case 'yml':
                ...
                if (strpos($vi, "\n") !== false || strpos($vi, "\r") !== false)
                {
                  $vi = str_replace(array("\r\n", "\r", "\n"), "\n            ", $vi);
                  $vi = "|\n            " . $vi;
                }

                $buffer .= "        " . trim($ki) . " : " . trim($vi) . "\n";
                break;
              case 'xml':
              default:
