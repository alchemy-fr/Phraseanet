...
user::updateClientInfos(5);	
?>
<html lang="<?php echo $session->usr_i18n;?>">
<head>
<title><?php echo p4string::MakeString(_('phraseanet:: thesaurus'))?></title>
...
// le lanceur nous passe la fen�tre du client
var wClient = null;
...
<div class="menu" id="kctermMenu" style="z-index:50; width:240px;">
	<a href="javascript:void(0)" class=""         id="kcterm_properties" style="font-weight:700"><?php echo p4string::MakeString(_('thesaurus::menu: proprietes')) /* Propri�t�s... */?></a>
	<a href="javascript:void(0)" class=""         id="kcterm_reject"><?php echo p4string::MakeString(_('thesaurus::menu: refuser')) /* refuser... */?></a> 
	<a href="javascript:void(0)" class="disabled" id="kcterm_accept"><?php echo p4string::MakeString(_('thesaurus::menu: accepter')) /* R�tablir comme candidat... */?></a> 
	<div class="line"></div>
...
<div class="menu" id="ktermMenu" style="z-index:50; width:200px;">
	<a href="javascript:void(0)" class="" id="kterm_properties" style="font-weight:700"><?php echo p4string::MakeString(_('thesaurus::menu: proprietes')) /* Propri�t�s... */?></a>
	<a href="javascript:void(0)" class="" id="kterm_newts"><?php echo p4string::MakeString(_('thesaurus::menu: Nouveau terme')) /* Nouveau terme sp�cifique... */?></a> 
		...
		<div id="TabT0" style="position:absolute; top:28px; left:0px; bottom:0px; width:40%;">
			...
			<div class="thbox" style="position:absolute; top:0px; bottom:8px; left:6px; right:3px;">
				...
				<div style="width:100%; overflow:hidden">
					<input type="checkbox" name="ckShowRejected" onClick="return(chgCkShowRejected());" /><span style="white-space:nowrap; overflow:hidden"><?php echo p4string::MakeString(_('thesaurus:: afficher les termes refuses')) /* Afficher les termes refus�s */?></span>
				</div>
...
-->	
			<div class="thbox"  style="position:absolute; top:0px; bottom:8px; left:3px; right:6px;">
				<div class="onglet"><span id='TabT1Title' style="cursor:pointer"><?php echo p4string::MakeString(_('thesaurus:: onglet thesaurus')) /* Th�saurus */?></span>
					&nbsp;
...
//					alert("id: "+o.id+" ; p: "+o.parentNode.id);
					...
					if(o.id.substr(4, 1)=="R")
					{
						...
						{
							// terme de premier niveau sous un champ : on peut r�tablir
							if(om=document.getElementById("kcterm_accept"))
								om.className = "";
						}
						else
						{
							// terme 'profond' (dans deleted) : on peut pas r�tablir
							if(om=document.getElementById("kcterm_accept"))
								om.className = "disabled";
						}
					}
					...
					if(o.firstChild.className == "nots")	// carr� en face du terme (+- ; balise 'U') == gris� ?
					{
						if(om = document.getElementById("kcterm_search"))
							om.className = "disabled";	// carr� grise
					}
					else
					{
						if(om=document.getElementById("kcterm_search"))
		...
		case "SELECT":
			switch(menuelem_id)
			{
				...
				case "kcterm_delete0hits":
					...
					if(n_nohits==0)
					{
						alert("<?php echo p4string::MakeString(_('thesaurus:: Tous les termes ont des hits'), "js") /* Cette branche ne contient aucun terme '� 0 hits' */?>");
						return;
					}
					else
					{
			...
			//		alert(url);
				...
				case "kcterm_properties":
					...
					break;
				case "kcterm_rescan":
					// if(confirm("<?php echo p4string::MakeString(_('thesaurus:: Supprimer tous les candidats et placer toutes les fiches en reindexation-thesaurus ?'), "js") /* Supprimer tous les candidats et placer toutes les fiches en r�indexation-th�saurus ? */?>"))
					// {
						var myObj = { "win":window };
						url  = "rescan_dlg.php";
						url += "?bid=<?php echo $parm["bid"]?>";
				...
				case "kcterm_export":
					...
					{
						url += "&dlg=1";
						// ret = window.showModalDialog(url, myObj, "dialogHeight:140px; dialogWidth:400px; center:yes; help:no; resizable:no; scroll:no; status:no; unadorned:yes");
						ret = window.showModalDialog(url, myObj, "dialogHeight:300px; dialogWidth:700px; center:yes; help:no; resizable:yes; scroll:yes; status:no; unadorned:yes");
					}
					else
					{
						// url += "&cb=" + encodeURIComponent("opener.newTS_Callback");
...
*/
					...
					case "THE_":		// le terme candidat
						myGUI.select(o);
						if(o.id.substr(4, 1) != "C")	// on ne peut pas draguer que les candidats, pas les refus�s ---
						{
							ret = false;
						}
						break;
  ...
  var ret;
	...
	switch(action)
	{
		case "INIT":
			...
			if(o.id.substr(4) == "T")
			{
				// menu contextuel � la racine du thesaurus
		...
		case "SELECT":
			switch(menuelem_id)
			{
				case "kterm_newts": // nouveau terme sp�cifique
					var myObj = { "win":window };
		...
		//				url += "&t=" + escape(newts);				// PAS avec un prompt UTF8
						...
						{
							w = window.open(url, "NEWT", "directories=no, height=290, width=490, location=no, menubar=no, resizable=yes, scrollbars=no, status=no, toolbar=no");
						}
					}
					break;
				case "kterm_newsy": 
		...
		//				url += "&t=" + escape(newts);				// PAS avec un prompt UTF8
						...
						{
							// url += "&cb=" + encodeURIComponent("opener.newTS_Callback");
							w = window.open(url, "NEWTS", "directories=no, height=290, width=490, location=no, menubar=no, resizable=yes, scrollbars=no, status=no, toolbar=no");
						}
					}
					break;
				case "kterm_delete":
				...
				case "kterm_link":
					...
					{
						w = window.open(url, "LINKFIELD", "directories=no, height=300, width=500, location=no, menubar=no, resizable=yes, scrollbars=no, status=no, toolbar=no");
					}
					break;
				...
				case "kterm_topics":
					...
					if(0)
					{
						url += "&dlg=1";
						// ret = window.showModalDialog(url, myObj, "dialogHeight:140px; dialogWidth:400px; center:yes; help:no; resizable:no; scroll:no; status:no; unadorned:yes");
						ret = window.showModalDialog(url, myObj, "dialogHeight:500px; dialogWidth:600px; center:yes; help:no; resizable:yes; scroll:yes; status:no; unadorned:yes");
					}
					else
					{
						// url += "&cb=" + encodeURIComponent("opener.newTS_Callback");
	...
	if(document.importNode)		// safari ? 
	{
		dstNode.innerHTML = "";
  		for(n=newContent.firstChild; n; n=n.nextSibling)
  			docImport(n, dstNode);	// assez rapide sous safari, tr�s tr�s lent sous explorer
	}
	else
	{
		var t = "";
		...
		dstNode.innerHTML = t;	// rapide sous explorer, tr�s tr�s lent sous safari
	}
}
...
//						lo.style.color="#ff00ff";
					...
					if(o.id.substr(0, 4) == "THE_") // && o.id != "THE_T")	// pas de drop � la racine du thesaurus
					{
						// myGUI.select(o);
