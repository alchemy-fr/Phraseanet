...
$sql = "SELECT server_coll_id FROM bas WHERE base_id='" . $conn->escape_string($parm["p1"])."'";
	...
	{		
		...
		if($connbas)
		{
			if($parm["act"]=="CHGSUGVAL")
			{
				...
				{		
					$sql = "UPDATE coll SET prefs='" . $connbas->escape_string($mdesc->saveXML()) . "' where coll_id='" .$connbas->escape_string($row["server_coll_id"])."'";
					$connbas->query($sql);
					$cache_app = cache_appbox::getInstance();
					$cache_app->delete('list_bases');
			...
			$sql = "SELECT prefs FROM coll where coll_id='".$connbas->escape_string($row["server_coll_id"])."'";
			if($rsbas = $connbas->query($sql))
			{
				if($rowbas = $connbas->fetch_assoc($rsbas))
				{
					$curPrefs = $rowbas["prefs"];
				}
...
<?php
$structfields =null;
if(($sxe = databox::get_sxml_structure($sbas_id)) !== false)
{
