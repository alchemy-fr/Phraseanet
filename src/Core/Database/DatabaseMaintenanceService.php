...
class DatabaseMaintenanceService
{
    ...
    //public function __construct(Application $application, Connection $connection, ContainerInterface $container)
    ...
    public function upgradeDatabase(\App\Utils\base $base, $applyPatches)
    {
        $recommends = [];
        $allTables = [];
    ...
    public function createTable(\SimpleXMLElement $table)
    {
        ...
        if ($table->defaults) {
            foreach ($table->defaults->default as $default) {
                ...
                $nonce = $this->container->get('random.lib')->getMediumStrengthGenerator()->generateString(16);
                ...
                foreach ($default->data as $data) {
                    ...
                    if ($k === 'usr_password') {
                        ...
                        $data = $this->container->get('auth.password-encoder')->encodePassword($data, $nonce);
                    }
